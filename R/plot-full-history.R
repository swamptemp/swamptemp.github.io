...
df <- df[order(df$dt), ]
df <- df[df$dt >=
         as.POSIXct(strptime("2017-08-28 14:00:00", "%Y-%m-%d %H:%M:%S")), ]
...
png(filename = "/home/pi/swamptemp.github.io/full-history.png",
...
g <- ggplot(plot.df, aes(dt, humidity)) + geom_line(aes(color = variable)) +
  ...
  theme(axis.text = element_text(family = "Roboto", size = 16),
        ...
        legend.text = element_text(family = "Roboto", size = 22),
        legend.position = "bottom", legend.title = element_blank())
png(filename = "/home/pi/swamptemp.github.io/full-humidity.png",
    height = 400, width = 800)
plot(g)
dev.off()
